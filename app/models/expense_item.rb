...
class ExpenseItem < ApplicationRecord
  ...
  def shares_sum_equals_amount
    return if amount_paise.nil? || amount_paise.zero?
    sum = expense_item_shares.sum { |s| s.amount_paise.to_i }
    return if sum == amount_paise
    errors.add(:base, "Item shares must total ₹#{amount_paise / 100.0}")
  end
end
